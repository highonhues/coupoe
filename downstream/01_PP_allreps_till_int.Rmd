...
Well htis just wont work here so i guess you gotta do this post cell subset?
...
Even before normal PCA I want to see if there are cell cycle effects.
...
# phase
seu <- RunPCA(seu, features = c(s.genes, g2m.genes), reduction.name="pca.cellcycle.before", reduction.key="CC_PC_B")

```

```{r}
DimPlot(seu, reduction="pca.cellcycle") #before
...
```{r}
seu <- RunPCA(seu, features = c(s.genes, g2m.genes), reduction.name="pca.cellcycle.after", reduction.key="CC_PC")
#just to viz
DimPlot(seu, reduction="pca.cellcycle") #before
