...
a <- LabelClusters(a, id = "seurat_clusters",  fontface = "bold", size = 5, bg.colour = "white", bg.r = .2, force = 0)
a
```
...
```{r}
a <- DimPlot(seu, group.by="cluster_annot") + ggtitle("Annotated Peyers Patches") +
  theme(plot.title = element_text(hjust = 0.5))
a <- LabelClusters(a, id = "cluster_annot",  fontface = "bold", size = 2.5, bg.colour = "white", bg.r = .2, force = 0)
a
save_pub(plot=a,filename="annone",width=10)
```
...
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 2] <- "CapEC Ifn"
...
a <- DimPlot(seu, group.by="cluster_annot") + ggtitle("Annotated Peyers Patches") +
  theme(plot.title = element_text(hjust = 0.5))
a <- LabelClusters(a, id = "cluster_annot",  fontface = "bold", size = 2.5, bg.colour = "white", bg.r = .2, force = 0)
save_pub(plot=a,filename="annone",width=9)
```
...
I need to see this mafb thing it might not even be venous but just CAP IFN
...
capifn <- c("Irf7","Cxcl9" ,"Cxcl10","Ifit1","Emcn" ,"Ly6a","Cdh13","Igfbp3","Ackr1","Vwf") #Ackr1 nahi hona chaiye in non Vn
```

ankr1 nahi hai,vwf nahi hai
```{r,fig.width=12,fig.height=11}
FeaturePlot(seu,capifn,label=T)
FeaturePlot(seu,CapEC1_marker,label=T)
```
...
```{r}
seu@meta.data$cluster_annot[seu@meta.data$cluster_annot == "Barrier EC"] <- "Stressed EC"
```
...
x <- VlnPlot(
  object = seu,
  features = "percent.mt",
  group.by = "cluster_annot", pt.size = 0
)
save_pub(plot=x,width = 10, height=4, filename="mtpt_stress_EC" )
```


```{r,fig.width=7,fig.height=5}
an <- DimPlot(seu,group.by="cluster_annot") + ggtitle("BECs of Peyers Patches") +
  theme(plot.title = element_text(hjust = 0.5))
an <- LabelClusters(an, id = "cluster_annot",  fontface = "bold", size = 4, bg.colour = "white", bg.r = .2, force = 0)
an
#save_pub(plot=an, filename="becann",width=6,height=5)
save_pub(plot=an, filename="ppt_clust",width=7,height=5)
```
...
Rename to capecIfn
```{r}
seu@meta.data$cluster_annot[seu@meta.data$cluster_annot == "Mafb+ Vn"] <- "CapEC Ifn"
```

I fear the trec are CapEC1

```{r}
seu@meta.data$cluster_annot[seu@meta.data$cluster_annot == "TrEC"] <- "CapEC1"
```
...
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 8] <- "Stressed EC"
...
an2 <- DimPlot(seu, split.by= "condition",group.by="cluster_annot",ncol=2) + ggtitle("Annotated Peyers Patches") +
  theme(plot.title = element_text(hjust = 0.5))
an2 <- LabelClusters(an2, id = "cluster_annot",  fontface = "bold", size = 4, bg.colour = "white", bg.r = .2, force = 0)
an2
save_pub(plot=an2, filename="spcon",width=10,height=9)
```
...
```{r}
seu <- subset(seu, subset = nFeature_RNA > 100 & nCount_RNA < 25000)
```


```{r,fig.width=10, fig.height=4}
p <- VlnPlot(seu, features = c("nCount_RNA"), group.by = "cluster_annot",
              pt.size = 0,
...
p <- VlnPlot(
  seu,
  features = c("Ackr1", "Hey1","Nr2f2"),
  split.by = "condition",
  pt.size = 0,idents = c("CapEC2", "Barrier EC","CRP"),
  ncol = 1
)

p & theme(legend.position = "right")  # use patchwork override
