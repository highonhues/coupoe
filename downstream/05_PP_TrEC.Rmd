...
celltype_dir <- file.path(base_dir, "PP_TrEC")
...
pseudobulk_celltype <- function(
  ...
  message("Cells in this cell type: ", ncol(seu))
  if (ncol(seu) < 50) warning("Very few cells — PCA & DGE may be unstable.")
  
  #----------------------
  # 2. Convert to SCE
  #----------------------
  sce <- as.SingleCellExperiment(seu, assay = assay_name)
...
diagnostic
```{r}
pb_sce <- pseudobulk_celltype(seu)
...
##DGE
```{r}
pb_pcv <- pseudobulk_celltype(seu, celltype = "TrEC")
...
```{r, fig.width=9, fig.height=7}
# Genes you care about
goi <- c("Madcam1", "St6gal1" ,"Nkx2-3", "Ccl21a","Ubd", "Fut7", "Oit1", "Clu" ,"Vwf" ,"Selp", "Sele", "Vcam1", "Glycam1", "Icam1","Nr2f2")
         #addressin                                                   
...
save_pub(plot=p,filename= "degcrpreprog", width = 9,height=7)
```
...
d <- dotplot(ego_bp, showCategory = 20) +
  theme(
    axis.text.y  = element_text(size = 22, face = "bold"),  # bigger + bold
...
d
save_pub(plot=d, filename ="crpora",width = 12, height=14)
```
...
p <- ggplot(top_ets, aes(x = NES, y = reorder(pathway, NES))) +
  ...
  labs(
    title = "ETS-related enrichment CRP (OE vs WT)",
...
Absolutely no relevant enrichment in CRP oe v wt!
...
for (pid in kegg_ids) {
  ...
  pathview(
    ...
    pathway.id = pid,
    species    = "mmu",
    out.suffix = paste0("OE_vs_WT_crp", pid)
  )
}
