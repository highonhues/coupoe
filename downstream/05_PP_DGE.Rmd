...
pseudobulk_celltype <- function(
  ...
  mutate(
    pseudo_rep = if_else(
      orig.ident %in% oe_samples,
      1L,   # OE → no pseudoreps
      { 
        # For CreCtrl, KO, WT → make 3 pseudoreps
        .rand <- sample(seq_len(n()))
        rep(1:n_pseudo, length.out = n())[order(.rand)]
      }
    )
  ) %>%
  ungroup()
  ...
  colData(sce)$pb_id <- ifelse(
    ...
    sce$orig.ident,   # OE stays OE1, OE2, OE3
    paste0(sce$orig.ident, "_rep", sce$pseudo_rep) # others get reps
  )
