...
p <- pheatmap(
  ...
  main = "Top 50 DEGs for PCV: OE vs KO"
)
...
msig_c3 <- msigdbr(species = "Mus musculus", collection =  = "C3") #, subcategory = "TFT:GTRD")
#msig_c2 <- msigdbr(species = "Mus musculus", category = "C2")
...
fgsea_ets <- fgsea_ets[grep("ETS|ERG|PEA3|ETV2|TCF|ERF|PDEF|ELF|ESE|TEL|SPI|ELG", fgsea_ets$pathway), ]
fgsea_ets <- fgsea_ets[order(fgsea_ets$pval), ]
fgsea_ets


```
Are these gene sets that have TF binding motifs in their promoter sites
...
#msig_c3_gtrd <- msigdbr(species = "Mus musculus", collection = "C3" , subcollection = "TFT:GTRD")
msig_c3_legacy <- msigdbr(species = "Mus musculus", collection = "C3" , subcollection = "TFT:TFT_LEGACY")
#msig_c2 <- msigdbr(species = "Mus musculus", category = "C2")


c3_tft <- split(msig_c3_legacy$gene_symbol, msig_c3$gs_name)
#c2_pathways <- split(msig_c2$gene_symbol, msig_c2$gs_name)
#library(fgsea)
...
fgsea_ets <- fgsea_ets[grep("ETS|ERG|ELK|FLI|ETV|GGA|ELG|ERF|ESE|ELF|PEA3", fgsea_ets$pathway), ]
# fgsea_ets <- fgsea_ets[order(fgsea_ets$pval), ]
fgsea_ets
...
p <- ggplot(top_ets, aes(x = NES, y = reorder(pathway, NES))) +
  ...
  scale_color_gradient2(low = "blue", high = "red", mid = "white") +
  labs(
    title = "ETS-related enrichment (OE vs KO)",
    x = "Normalized Enrichment Score (NES)",
    y = "ETS / ERG / ELK / ETV target sets",
    size = "-log10(FDR)"
  ) +
