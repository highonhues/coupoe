...
celltype_dir <- file.path(base_dir, "PP_Artery")
...
##DGE
```{r}
pb_pcv <- pseudobulk_celltype(seu, celltype = "Artery")
...
p <- EnhancedVolcano(
  ...
  widthConnectors = 0.5,
  colConnectors = 'black',
  title = 'OE vs WT - Adhesion Molecules Artery'
)
...
p <- EnhancedVolcano(
  ...
  widthConnectors = 0.5,
  colConnectors = 'black',
  title = 'Is There Reprogramming of CapEC2 to HEV? OE vs WT'
)

save_pub(plot=p,filename= "deg_oewtec2reprog", width = 9,height=7)
```
...
p <- pheatmap(
  ...
  main = "Top 50 DEGs for Art: OE vs WT"
)
...
p <- ggplot(top_ets, aes(x = NES, y = reorder(pathway, NES))) +
  ...
  labs(
    title = "ETS-related enrichment Artery (OE vs WT)",
...
Absolutely no relevant enrichment in art oe v wt!
...
for (pid in kegg_ids) {
  ...
  pathview(
    ...
    pathway.id = pid,
    species    = "mmu",
    out.suffix = paste0("OE_vs_WT_art", pid)
  )
}
...
p <- EnhancedVolcano(
  ...
  widthConnectors = 0.5,
  colConnectors = 'black',
  title = 'KO vs WT - Adhesion Molecules Artery'
)
...
p <- pheatmap(
  ...
  main = "Top 50 DEGs for CapEC2: KO vs WT"
)
...
p <- ggplot(top_ets, aes(x = NES, y = reorder(pathway, NES))) +
  ...
  scale_color_gradient2(low = "blue", high = "red", mid = "white") +
  labs(
    title = "ETS-related enrichment CapEC2 (KO vs WT)",
    x = "Normalized Enrichment Score (NES)",
    y = "ETS target sets",
    size = "-log10(FDR)"
  ) +
...
for (pid in kegg_ids) {
  ...
  pathview(
    ...
    pathway.id = pid,
    species    = "mmu",
    out.suffix = paste0("KO_vs_WT_capec2", pid)
  )
}
