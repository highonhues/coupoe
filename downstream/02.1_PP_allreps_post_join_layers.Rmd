...
DimPlot(seu, grou = "umap.mnn", group.by = "Phase") + ggtitle("Viewing Cell cycle Effects on Clusters") +
  theme(plot.title = element_text(hjust = 0.5))
```
```{r}
levels(seu)
...
```
```{r}
sum(seu$mnn_clusters == seu$seurat_clusters) #yuh for all 20333 cells its equal
```


```{r, fig.height=6, fig.width=12}

DimPlot(seu, group.by="seurat_clusters", split.by = "condition") + ggtitle("Viewing Cells in each cluster Split By Condition") +
  theme(plot.title = element_text(hjust = 0.5))
```
```{r}
...
saveRDS(
  object = seu,
  file = file.path(rds_dir, "seu_pp_int_fin.rds")) # just in case
...
I want to now move on to filtering the contaminants. Add module score is able to avergage multiple genes. Lets do it in a new rmd,shall we?
