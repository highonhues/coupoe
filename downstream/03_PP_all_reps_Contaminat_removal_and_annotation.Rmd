...
capifn <- c("Irf7", "Cxcl10","Ifit1" ,"Ackr1") #Ackr1 nahi hona chaiye in non Vn
``` 
...
22109 features across 19035 samples within 2 assays 
Active assay: mnn.reconstructed (2000 features, 2000 variable features)
...
p <- FeaturePlot(seu, features = c("LEC", "Lymphocyte", "FRC"),label=T,
            #cols = c("lightgrey", "red"),  # optional: nice contrast
            reduction = "umap.mnn")
save_pub(plot=p, filename="fpcont",width=8)
```
...
save_pub(plot=p, filename="vlncont",width=12, height=4)
```
...
save_pub(plot=p, filename="dpcont",width=7, height=7)
```
...
save_pub(plot=p, filename="thrcont")
```
...
```{r}
save_pub(plot=p,filename= "inires",width=10,height=8)
```



```{r,fig.height=15,fig.width=7}
p <- Seurat::DimPlot(seu_obj, group.by = c("RNA_snn_res.0.1","RNA_snn_res.0.2", "RNA_snn_res.0.3"),label=TRUE, ncol=1)
save_pub(plot=p,filename= "inires",width=7,height=15)
```
...
```{r}
p <- FeaturePlot(seu_obj,hev_markers,label=T)
save_pub(plot=p, filename = "hevini",width=8,height=8)
