...
celltype_dir <- file.path(base_dir, "PP_CapEC_Ifn")
...
I want to check if Mafb is Cap Ifn
...
##DGE
```{r}
pb_pcv <- pseudobulk_celltype(seu, celltype = "CapEC Ifn")
...
CRE_CTRL vs WT i j wanna see 


```{r}
# 
write.csv(deg_cc_wt,
          file.path(celltype_dir, "deg_cc_wt.csv"),
          row.names = TRUE)

# How many significant genes?
sig_genes <- deg_cc_wt %>% filter(FDR < 0.05)
nrow(sig_genes)
...
write.csv(ego_bp,
          file.path(celltype_dir, "ego_bp_deg_cc_wt.csv"),
          row.names = FALSE)
...
save_pub(plot =b, filename="barplot_GO_deg_cc_wt", width=14, height=9)
save_pub(plot =d, filename="dotplot_GO_deg_wt_wt", width=14, height=9)
```
...
save_pub(plot=p,filename= "deg_oe_wt_goi_EVplot_capifn", width = 9,height=7)
```
...
p <- pheatmap(
  ...
  main = "Top 50 DEGs for CapEC Ifn: OE vs WT"
)
...
p <- ggplot(top_ets, aes(x = NES, y = reorder(pathway, NES))) +
  ...
  labs(
    title = "ETS-related enrichment CapEC lfn (OE vs WT)",
...
Absolutely no relevant enrichment in Capec lfn oe v wt!
...
```
Nothing. so we will just move on we have all we need for cap Ifn.
...
save_pub(plot=p,filename= "deg_ko_wt_goi_EVplot_capifn", width = 9,height=7)
```
