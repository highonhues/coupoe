...
check karo does cell cell effect get removed? THEY DOOOO WHATTTT
...
```{r}
# View resuts
p <- DimPlot(seu, group.by="seurat_clusters") 
#p <- LabelClusters(p, id = "seurat_clusters",  fontface = "bold", size = 5, bg.colour = "white", bg.r = .2, force = 0)
#p <- p+ ggtitle("Initial Integration for MLN") + theme(plot.title = element_text(hjust = 0.5))
```

```{r}
save_pub(plot=p,  filename= "MLN_init_int_no_label", width=7, height=7)
```

```{r,fig.width=9,fig.height=9}
p <- DimPlot(seu, reduction = "umap.mnn", split.by = "condition", ncol =2)
save_pub(plot=p,  filename= "MLN_umap,split_by_cond", width=9, height=9)
```
