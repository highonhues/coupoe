...
save_pub(plot=p, filename="MLN_Clust_Finding_res",width =7, height=15)
```
I'm torn between 0.2 and 0.3?? I wanna try both I think bec they both have the same resolution.


Let's start with 0.3 first


```{r}

Idents(seu_obj) <- "RNA_snn_res.0.3"
```

```{r}

markers_all_03res <- FindAllMarkers(
  seu_obj,
  only.pos = TRUE,
  min.pct = 0.25,
...
```{r}
write.csv(markers_all_03res,
          file = "MLN_outputs/markers_all_03res.csv",
          row.names = FALSE)
...
```{r,fig.width=9}
FeaturePlot(seu_obj,c("Nr2f2","Madcam1","Acta2"),label=T)
```
8 is showing some pericyte endothlial cell str lemme check if its a doublet. deosnt look it ok 
...
so lets cluster at 0.3 and rename and then new
...
p <- DimPlot(seu, group.by="seurat_clusters",)  + ggtitle("MLN Clusters Identified") +
  theme(plot.title = element_text(hjust = 0.5))
#p <- p +LabelClusters(p, id = "seurat_clusters",  fontface = "bold", size = 5, bg.colour = "white", bg.r = .2, force = 0)
```

```{r}
save_pub(plot=p, filename="MLN_clust_nolabel")
```
CHECK WHAT SEPARATES LEC1 AND 2
...
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 0] <- "Madcam1+ HEV"
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 1] <- "Aqp7+ CapEC/ CapEC1"
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 2] <- "Artery"
...
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 5] <- "LEC1"
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 6] <- "Pericytes"
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 7] <- "Unknown EC"
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 8] <- "Pericyte EC"
seu@meta.data$cluster_annot[seu@meta.data$seurat_clusters == 9] <- "LEC2"
...
```{r}
an <- DimPlot(seu, group.by="cluster_annot") + ggtitle("Annotated MLN") +
  theme(plot.title = element_text(hjust = 0.5))
...
FeaturePlot(seu_obj,c("Nr2f2","Madcam1","Sele"),label=T)
```
